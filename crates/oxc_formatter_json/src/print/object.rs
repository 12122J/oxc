...
use crate::{
    comments::{
        FormatLeadingComments, FormatTrailingInsideComments, is_suppressed_before,
        write_dangling_comments, write_suppressed_node,
    },
...
use super::{
    FmtJsonValue, JsonFormatter, arena_cow_str, format_with, literal::FmtJsonString,
    write_source_slice,
};
...
impl<'a> Format<'a, JsonFormatContext<'a>> for FmtJsonObject<'a, '_> {
    fn fmt(&self, f: &mut JsonFormatter<'_, 'a>) {
        ...
        if self.object.properties.is_empty() {
            let dangling = f.context().take_comments_before(self.object.span.end);
            ...
            write_separated(f, &spans, TrailingSeparator::Disallowed, |i, f| {
                ...
                match property {
                    ObjectPropertyKind::ObjectProperty(prop) => {
                        // Property-level `oxfmt-ignore` / `prettier-ignore`:
                        // emit the whole `key: value` slice verbatim.
                        if is_suppressed_before(f, prop.span.start) {
                            write!(f, FormatLeadingComments(prop.span));
                            write_suppressed_node(prop.span, f);
                        } else {
                            write!(f, FormatLeadingComments(prop.span));
                    ...
                    ObjectPropertyKind::SpreadProperty(spread) => {
                        write_source_slice(spread.span, f);
                    }
                }
            });
        ...
        //
        // `objectWrap` controls the multi-line shape preservation: with `Auto`
        // (preserve), an object whose source has a newline right after `{` stays
        // expanded; with `Never` (collapse), the group decides purely by width.
        // The array side does NOT do this — only objects.
        let options = f.context().options();
        let expand = match options.expand {
            Expand::Auto => opens_on_new_line(self.object, f.context().source()),
            Expand::Never => false,
        };
        write!(
            f,
...
/// - Numeric keys are normalized via [`format_trimmed_number`]; whether the result is
///   quoted follows Prettier's `shouldQuotePropertyKey` rule
///   (mirrors `language-js/print/property.js:shouldQuotePropertyKey`): a number key
///   is quoted only when its normalized form is a "simple number" *and* round-trips
///   through `f64` losslessly. Otherwise it's emitted bare.
...
fn write_object_key<'a>(key: &PropertyKey<'a>, f: &mut JsonFormatter<'_, 'a>) {
    match key {
        ...
        // Other PropertyKey variants (`PrivateIdentifier`, computed `[expr]`, etc.)
        // are not valid in JSON. Fall back to the source slice as a last resort.
        _ => write_source_slice(key.span(), f),
    }
}
...
/// 2. it equals `String(Number(printed))` — i.e. survives an f64 round-trip
