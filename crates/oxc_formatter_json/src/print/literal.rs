...
impl<'a> Format<'a, JsonFormatContext<'a>> for FmtJsonString<'a, '_> {
    fn fmt(&self, f: &mut JsonFormatter<'_, 'a>) {
        ...
        // Detect the outer quote. Anything unexpected (no surrounding quotes,
        // mismatched delimiters) falls back to verbatim source.
...
impl<'a> Format<'a, JsonFormatContext<'a>> for FmtJsonNumber<'a, '_> {
    fn fmt(&self, f: &mut JsonFormatter<'_, 'a>) {
        ...
        // `keep_one_trailing_decimal_zero` matches Prettier's JS/JSON behavior
        // (`1.00000` → `1.0`, not `1`).
...
/// Returns the outer quote of `raw` if it's a valid quoted-string literal
/// (matching `'…'` or `"…"`), otherwise `None`.
