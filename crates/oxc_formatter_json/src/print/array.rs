...
use super::{FmtJsonValue, JsonFormatter, format_with, write_source_slice};
...
impl<'a> Format<'a, JsonFormatContext<'a>> for FmtJsonArray<'a, '_> {
    fn fmt(&self, f: &mut JsonFormatter<'_, 'a>) {
        ...
        if self.array.elements.is_empty() {
            let dangling = f.context().take_comments_before(self.array.span.end);
        ...
        // Sparse-only arrays like `[,]`, `[, , ,]` would produce empty fill/group
        // entries (Elision emits nothing). Group builders assert on empty content,
        // so emit the source slice verbatim — it already captures the user's holes.
        ...
        // When the last element is an Elision (e.g. `[1, , 2,,,,]`), the trailing `,`
        // is load-bearing for the hole count. Otherwise (`[1, 2, 3,]`) the trailing
        // comma is just JSON5 syntax that Prettier's `json` parser strips.
        let trailing_comma =
            matches!(self.array.elements.last(), Some(ArrayExpressionElement::Elision(_)));

        // Fill packs multiple numeric entries on one line. The trailing `,` lives on
        // the item (mirrors Prettier's `printArrayElementsConcisely`) so the fill
        // measurer counts it toward "does this item fit on the current line?" — without
        // that, the comma trailing the last on-line item gets pushed past `line_width`.
        ...
        if can_concisely_print(&self.array.elements) {
            ...
            let body = format_with(|f: &mut JsonFormatter<'_, 'a>| {
                ...
                // `elements` is non-empty in this branch (the empty-array early return
                // is above), so `last()` cannot be `None`.
        ...
        // and every element has at least 2 inner items. Matrices and arrays-of-records
        // qualify; `[[1, 2], [3]]` or a single-element wrapper does not.
...
/// Writes one array element. Elision (`[1, , 2]` holes) emits nothing — the
/// surrounding separator alone represents the hole. SpreadElement isn't valid
/// JSON; we fall back to its source slice as a last resort.
fn write_array_element<'a>(element: &ArrayExpressionElement<'a>, f: &mut JsonFormatter<'_, 'a>) {
    ...
    if let Some(expr) = element.as_expression() {
        ...
        write_source_slice(element.span(), f);
    }
}

/// Returns `true` if the array elements are all numeric literals and can be "fill-printed".
///
/// Accepts both bare numbers (`1`, `0.5`) and signed numbers (`-1`, `+0.5`) — the JS
/// parser models the latter as `UnaryExpression(±, NumericLiteral)`, so omitting it
/// would knock common JSON shapes like coordinate or bounding arrays out of fill mode.
