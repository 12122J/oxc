...
/// Whether objects keep their authored multi-line shape or collapse to one line
/// when they fit. Mirrors Prettier's `objectWrap` option for the `json` parser.
#[derive(Debug, Default, Clone, Copy, Eq, PartialEq)]
pub enum Expand {
    /// `objectWrap: "preserve"`. An object stays multi-line if there's a newline
    /// after `{` in the source; otherwise it collapses when it fits.
    #[default]
    Auto,
    /// `objectWrap: "collapse"`. Objects collapse when they fit regardless of the
    /// authored shape.
    Never,
}
...
pub struct JsonFormatOptions {
    ...
    /// Whether to print a single space inside object braces when the object fits
    /// on one line: `{ "a": 1 }` vs `{"a": 1}`. Mirrors Prettier's `bracketSpacing`.
    pub bracket_spacing: bool,
    /// Object multi-line preservation strategy. See [`Expand`].
    pub expand: Expand,
}
