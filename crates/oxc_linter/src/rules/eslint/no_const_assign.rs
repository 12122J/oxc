...
fn test() {
    ...
    let fail = vec![
        ...
        "x = 123; const x = 1;",
        // Binding patterns
        "const [a, b, ...[c, ...d]] = [1, 2, 3, 4, 5]; d = 123",
        "const d = 123; [a, b, ...[c, ...d]] = [1, 2, 3, 4, 5]",
        "const b = 0; ({a, ...b} = {a: 1, c: 2, d: 3})",
        // using + await using
        "using x = foo(); x = 1;",
        "await using x = foo(); x = 1;",
        "using x = foo(); x ??= bar();",
        "await using x = foo(); x ||= bar();",
        "using x = foo(); [x, y] = bar();",
        "await using x = foo(); [x = baz, y] = bar();",
        "using x = foo(); ({a: x} = bar());",
        "await using x = foo(); ({a: x = baz} = bar());",
    ];
